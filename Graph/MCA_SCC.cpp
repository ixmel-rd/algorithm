class SCC { // cmp(0 - kk-1) is scc's return. Same number is group. The number
            // is no relatoin.
  public:
    int n, N;
    vvi G, rG;
  ...
  public:
    ...
    int mca() {
        ...
        if (h) { // connect
            ...
            int sum = 0;
            rep(i, n)
                rep(j, tG[i].size()) if (scc.cmp[i] == scc.cmp[tG[i][j].to])
                    sum += tG[i][j].cost;
            rep(i, n)
                rep(j, G[i].size()) if (scc.cmp[i] != scc.cmp[G[i][j].to]) {
                bool h = false;
                int To = G[i][j].to; // i -> To
                rep(k, n) if (To != k && scc.cmp[To] == scc.cmp[k]) h = true;
                if (h) {
                    mca.add_edge(scc.cmp[i], scc.cmp[To],
                                 G[i][j].cost - trG[To][0].cost);
                } else {
