...
class SegTree {
  public:
    ...
    SegT get(int x) { return val[n - 1 + x]; }
    void tmp() {
...
class SegTree_2d {
  public:
    ...
    SegTree_2d(const vector<vector<SegT>> &in) {
        ...
        for (int i = n - 1 + in.size() - 1; i >= 0; i--) {
            if (i - (n - 1) < 0)
                rep(j, val[i].val.size()) val[i].val[j] =
                    combine(val[i * 2 + 1].val[j], val[i * 2 + 2].val[j]);
            else
                val[i] = SegTree(in[i - (n - 1)]);
        }
    }
    ...
    SegT query(int x1, int y1, int x2, int y2, int k = 0, int l = 0,
               int r = -1) { //[a,b)
        if (r == -1)
            r = n;
        if (r <= x1 || x2 <= l)
            return defvalue;
        if (x1 <= l && r <= x2)
            return val[k].query(y1, y2);
        else
            return combine(query(x1, y1, x2, y2, k * 2 + 1, l, (l + r) / 2),
                           query(x1, y1, x2, y2, k * 2 + 2, (l + r) / 2, r));
    }
    SegT get(int x, int y) { return val[n - 1 + x].get(y); }
    void tmp() { rep(i, val.size()) val[i].tmp(); }
};
int main() {
    int n, m, T, q;
    cin >> n >> m >> T >> q;
    ...
    while (q--) {
        ...
        if (c == 0) {
            ...
            cout << st2.query(x1, y1, x2, y2) << " "
                 << st1.query(x1, y1, x2, y2) << endl;
        }
    }
}
