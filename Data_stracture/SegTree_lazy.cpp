  ...
  public:
    ...
    LSTree(const vector<SegT> &v) {
        ...
        for (int i = n - 1 + v.size() - 1; i >= 0; i--) {
            if (n - 1 <= i)
                ...
                node[i] = combine(node[i * 2 + 1], node[i * 2 + 2]);
        }
    }
    ...
    SegT query(int a, int b, int k = 0, int l = 0, int r = -1) {
        ...
        if (a <= l && r <= b)
            ...
            return combine(query(a, b, 2 * k + 1, l, (l + r) / 2),
                           query(a, b, 2 * k + 2, (l + r) / 2, r));
    }
    void tmp() {
        rep(i, node.size()) cout << " " << node[i];
        cout << endl;
        rep(i, node.size()) cout << " " << lazy[i];
        cout << endl;
        cout << endl;
    }
};
