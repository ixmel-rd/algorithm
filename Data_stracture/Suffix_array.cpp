...
template <class T>
void induce(int n, const vector<T> &s, vector<bool> &ls, vi &sa, vi &l_basm,
            int B) {
    vi buf = l_basm;
    rep(i, n + 1) {
        if (sa[i] >= 1 && !ls[sa[i] - 1]) {
...
template <class T> void SA(int n, const vector<T> &s, vi &sa, int B = 200) {
...
template <class T> void LCP(int n, const vector<T> &s, const vi &sa, vi &lcp) {
    vi rsa(n + 1);
...
int main() {
    ...
    ll out = 0;
    loop(i, 1, n + 1) out += (long long)(n - sa[i]) * (n - sa[i] + 1) / 2 -
                             (ll)lcp[i] * (lcp[i] + 1) / 2;
    cout << out << endl;
}
