class CHT { // http://satanic0258.hatenablog.com/entry/2016/08/16/181331
  public:
    ...
    bool comp(Def l, Def r) { return l >= r; }

    // コンストラクタ ( クエリが単調であった場合はflag = trueとする )
    // compFunc (初期値はmin) l<=rならmax
    CHT(bool flagX = false) { isMonotonicX = flagX; };
    ...
    Def f(Def i, Def x) { return lines[i].first * x + lines[i].second; }
    // i番目の直線f_i(x)に対するxの時の値を返す
    Def f(pii line, Def x) { return line.first * x + line.second; }
